...
from .tools import ustr, consteq, frozendict, unique, date_utils
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if "Controller" not in globals() or Controller not in bases:
            return
...
def session_gc_default(session_store):
    ...
    def __call__(self, environ, start_response):
        def start_wrapped(status, headers):
            ...
            if (
                ...
                and "wkhtmltopdf" not in req.headers.get("User-Agent")
            ):
