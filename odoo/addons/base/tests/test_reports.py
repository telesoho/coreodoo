...
class TestReports(odoo.tests.TransactionCase):
    ...
    def test_barcode_check_digit(self):
        ean8 = "87111125"
        self.assertEqual(
            self.env["ir.actions.report"].get_barcode_check_digit("0" * 10 + ean8),
            int(ean8[-1]),
        )
        ean13 = "1234567891231"
        self.assertEqual(
            self.env["ir.actions.report"].get_barcode_check_digit("0" * 5 + ean13),
            int(ean13[-1]),
        )

    def test_barcode_encoding(self):
        self.assertTrue(
            self.env["ir.actions.report"].check_barcode_encoding("20220006", "ean8")
        )
        self.assertTrue(
            self.env["ir.actions.report"].check_barcode_encoding("93855341", "ean8")
        )
        self.assertTrue(
            self.env["ir.actions.report"].check_barcode_encoding(
                "2022071416014", "ean13"
            )
        )
        self.assertTrue(
        ...
        self.assertFalse(
            self.env["ir.actions.report"].check_barcode_encoding("2022a006", "ean8"),
            "should contains digits only",
        )
        self.assertFalse(
            self.env["ir.actions.report"].check_barcode_encoding("20220000", "ean8"),
            "incorrect check digit",
        )
        self.assertFalse(
            self.env["ir.actions.report"].check_barcode_encoding("93855341", "ean13"),
